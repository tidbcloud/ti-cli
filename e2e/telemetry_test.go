...
func TestTelemetryDeliveryToTiDB(t *testing.T) {
	if os.Getenv("TI_TELEMETRY_E2E") != "1" {
		t.Skip("run make telemetry-e2e")
	}
	baseDSN := strings.TrimSpace(os.Getenv("TI_TEST_TELEMETRY_TIDB_DSN"))
	if baseDSN == "" {
		t.Fatal("TI_TEST_TELEMETRY_TIDB_DSN is required")
	}
...
flag_names_json, exit_code, duration_ms, cli_version, os, arch, schema_version
		...
		"legacy-event", "ti_legacy_installation", "ti.command.finished", "ti legacy", "0.0.0", "linux", "amd64"); err != nil {
	...
	result := runTIWithInput(t, tiBinary(t), "", append(tiConfigEnv(),
		...
		"TI_TEST_TELEMETRY_ENDPOINT="+endpoint+"/v1/telemetry/batch",
		"TI_TELEMETRY=on",
		"TI_TELEMETRY_TAG="+tag,
		`TI_TELEMETRY_EXTRA={"telemetry_e2e_run":"`+runID+`"}`,
...
func createTelemetryE2EDatabase(t *testing.T, ctx context.Context, baseDSN string) (string, string) {
	...
	if err != nil {
		t.Fatal("parse TI_TEST_TELEMETRY_TIDB_DSN")
	}
	adminConfig := *config
	adminConfig.DBName = ""
	...
	name := "ti_telemetry_e2e_" + strings.ReplaceAll(time.Now().UTC().Format("20060102150405000000000"), "-", "")
...
func telemetryMigratorBinary(t *testing.T) string {
	t.Helper()
	path := strings.TrimSpace(os.Getenv("TI_TELEMETRY_MIGRATOR_E2E_BIN"))
	if path == "" {
		t.Fatal("TI_TELEMETRY_MIGRATOR_E2E_BIN is required; run make telemetry-e2e")
	}
	return path
}
...
func assertLegacyEventSurvivesMigration(t *testing.T, ctx context.Context, db *sql.DB) {
	...
	if commandPath != "ti legacy" || tag != "" || extra != nil {
...
func startTelemetryBackend(t *testing.T, ctx context.Context, dsn, postHogURL string) string {
	...
	if binary == "" {
		t.Fatal("TI_TELEMETRY_BACKEND_E2E_BIN is required; run make telemetry-e2e")
	}
...
FROM telemetry_events WHERE tag = ? ORDER BY received_at DESC LIMIT 1`, tag).Scan(&commandPath, &exitCode, &schema, &extra)
	...
	if commandPath != "ti db create-db-cluster" || exitCode != 0 || schema != 2 || metadata["telemetry_e2e_run"] != runID {
