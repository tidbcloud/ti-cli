...
func newFSCopyFileCommand(info version.Info) *cobra.Command {
	...
	cmd.Flags().Bool("recursive", false, "Copy directory structure recursively; cannot be combined with --layer-id. Seed a layer through a writable FUSE mount instead.")
	cmd.Flags().Bool("resume", false, "Resume an active copy operation.")
	cmd.Flags().String("layer-id", "", "Write one copied file into a file system layer instead of the base file system; cannot be combined with --recursive.")
...
func newFSForkLayerCommand(info version.Info) *cobra.Command {
	cmd := newControlPlaneCommand(controlPlaneCommandSpec{
		Use:        "fork-layer",
		Short:      "Fork a copy-on-write child file system layer. (preview)",
		Mutation:   mutatingCommand,
		Permission: authz.FSFileWrite,
		Run: func(ctx commandContext) (any, error) {
			service, profile, err := fsServiceAndProfile(ctx)
			...
			opts, err := fsForkLayerOptions(ctx, profile)
			if err != nil {
				return nil, err
			}
			return service.ForkLayer(ctx.cmd.Context(), opts)
		},
		DryRun: func(ctx commandContext) (dryrun.Result, error) {
			service, profile, err := fsServiceAndProfile(ctx)
			if err != nil {
			...
			if err := tifs.ValidateLayerReference(opts.ParentLayerRef, "--parent-layer-ref"); err != nil {
				return dryrun.Result{}, err
			}
			if opts.CheckpointID != "" {
				if err := tifs.ValidateLayerReference(opts.CheckpointID, "--checkpoint-id"); err != nil {
					return dryrun.Result{}, err
				}
			}
			return service.DryRunLayerMutation(ctx.cmd.Context(), ctx.CommandPath(), "fork_layer", "POST", "/v1/layers/"+opts.ParentLayerRef+"/fork", map[string]any{
				"layer_id":      opts.LayerID,
				"name":          opts.LayerName,
				"checkpoint_id": opts.CheckpointID,
				"actor_id":      opts.ActorID,
			}, profile, authz.FSFileWrite, tifs.LayerCapabilityFork)
		},
	}, info)
	cmd.Flags().String("parent-layer-ref", "", "The parent layer ID, unique name, or supported tag reference.")
	cmd.Flags().String("layer-id", "", "The child layer ID. Normally it is generated by the service automatically.")
	cmd.Flags().String("layer-name", "", "The name of the child layer.")
	cmd.Flags().String("checkpoint-id", "", "Pin the child to this checkpoint of the parent layer.")
	cmd.Flags().String("actor-id", "", "Actor ID identifying the child layer owner.")
	markUsageRequired(cmd, "parent-layer-ref")
	return cmd
}

func newFSListLayerChainCommand(info version.Info) *cobra.Command {
	cmd := newControlPlaneCommand(controlPlaneCommandSpec{
		Use:        "list-layer-chain",
		Short:      "List a file system layer ancestry chain. (preview)",
		Mutation:   readOnlyCommand,
		Permission: authz.FSFileRead,
		Run: func(ctx commandContext) (any, error) {
			service, profile, err := fsServiceAndProfile(ctx)
			...
			layerRef, err := ctx.StringFlag("layer-ref")
			if err != nil {
				return nil, err
			}
			return service.ListLayerChain(ctx.cmd.Context(), tifs.ListLayerChainOptions{Profile: profile, LayerRef: layerRef})
		},
	}, info)
	cmd.Flags().String("layer-ref", "", "The layer ID, unique name, or supported tag reference.")
	markUsageRequired(cmd, "layer-ref")
	return cmd
}
...
func newFSDeleteLayerCommand(info version.Info) *cobra.Command {
	cmd := newControlPlaneCommand(controlPlaneCommandSpec{
		Use:        "delete-layer",
		Short:      "Abandon a file system layer. (preview)",
		Mutation:   mutatingCommand,
		Permission: authz.FSFileWrite,
		Run: func(ctx commandContext) (any, error) {
			service, profile, err := fsServiceAndProfile(ctx)
			...
			opts, err := fsDeleteLayerOptions(ctx, profile)
			if err != nil {
				return nil, err
			}
			return service.DeleteLayer(ctx.cmd.Context(), opts)
		},
		DryRun: func(ctx commandContext) (dryrun.Result, error) {
			service, profile, err := fsServiceAndProfile(ctx)
			if err != nil {
			...
			if err := tifs.ValidateLayerReference(opts.LayerRef, "--layer-ref"); err != nil {
				return dryrun.Result{}, err
			}
			return service.DryRunLayerMutation(ctx.cmd.Context(), ctx.CommandPath(), "delete_layer", "DELETE", fmt.Sprintf("/v1/layers/%s?cascade=%t", opts.LayerRef, opts.Cascade), nil, profile, authz.FSFileWrite, tifs.LayerCapabilityDelete)
		},
	}, info)
	cmd.Flags().String("layer-ref", "", "The layer ID, unique name, or supported tag reference.")
	cmd.Flags().Bool("cascade", false, "Abandon live descendants before abandoning this layer.")
	markUsageRequired(cmd, "layer-ref")
	return cmd
}
...
func newFSMountFileSystemCommand(info version.Info) *cobra.Command {
	...
	cmd.Flags().Bool("write-back-cache", true, "Persist FUSE writes locally before writing them to the file system on flush; unavailable for checkpoint mounts.")
	cmd.Flags().String("mount-profile", "", "Mount profile: coding-agent, portable, or none. Default: none.")
	...
	cmd.Flags().String("layer-ref", "", "Mount through this writable layer ID, unique name, or supported tag reference; requires FUSE.")
	cmd.Flags().String("checkpoint-id", "", "Mount this checkpoint of --layer-ref read-only; requires FUSE.")
...
func fsForkLayerOptions(ctx commandContext, profile *config.Profile) (tifs.ForkLayerOptions, error) {
	parentLayerRef, err := ctx.StringFlag("parent-layer-ref")
	if err != nil {
		return tifs.ForkLayerOptions{}, err
	}
	layerID, err := ctx.StringFlag("layer-id")
	if err != nil {
		return tifs.ForkLayerOptions{}, err
	...
	return tifs.ForkLayerOptions{Profile: profile, ParentLayerRef: parentLayerRef, LayerID: layerID, LayerName: layerName, CheckpointID: checkpointID, ActorID: actorID}, nil
}

func fsDeleteLayerOptions(ctx commandContext, profile *config.Profile) (tifs.DeleteLayerOptions, error) {
	layerRef, err := ctx.StringFlag("layer-ref")
	if err != nil {
		return tifs.DeleteLayerOptions{}, err
	}
	cascade, err := ctx.BoolFlag("cascade")
	if err != nil {
		return tifs.DeleteLayerOptions{}, err
	}
	return tifs.DeleteLayerOptions{Profile: profile, LayerRef: layerRef, Cascade: cascade}, nil
}
...
func fsMountOptions(ctx commandContext, profile *config.Profile) (tifs.MountFileSystemOptions, error) {
	...
	return tifs.MountFileSystemOptions{
		...
		Driver:            driver,
		ReadOnly:          readOnly,
		ReadOnlySet:       ctx.FlagChanged("read-only"),
		...
		WriteBackCacheSet: ctx.FlagChanged("write-back-cache"),
		MountProfile:      mountProfile,
		LocalRoot:         localRoot,
		PackPaths:         packPaths,
